...
player2Score.src = "sounds/comScore.mp3";
player1Score.src = "sounds/userScore.mp3";
...
// listening to the imu
function updatePlayerPosition(){

    const SERVERADDRESS1 = "http://127.0.0.1:3000";
    const SERVERADDRESS2 = "http://127.0.0.1:3001";
    var data = "";
	...
	xmlhttp1.onreadystatechange = function () {
		if (xmlhttp1.readyState == 4 && xmlhttp1.status == 200) {
			...
			// calculate pitch 
            let pitch = 1 * 180 * Math.atan2(sensorValueX, Math.sqrt(sensorValueY*sensorValueY + sensorValueZ*sensorValueZ))/Math.PI;
            
            console.log(player1.y);

            // make the user paddle exist between 350 and -50
            let newUserY = player1.y + 0.25*pitch;
            if (newUserY >= 350) {
                newUserY = 350;
            }
            else if (newUserY <= -50) {
    ...
    xmlhttp2.onreadystatechange = function () {
		if (xmlhttp2.readyState == 4 && xmlhttp2.status == 200) {
            ...
            // make the user paddle exist between 350 and -50
            let newUserY = player2.y + 0.25*pitch;
            if (newUserY >= 350) {
                newUserY = 350;
            }
            else if (newUserY <= -50) {
	...
	let getData = () => {
		xmlhttp1.open("POST", SERVERADDRESS1, true);
...
// when a player scores, we reset the ball
function resetBall(){
